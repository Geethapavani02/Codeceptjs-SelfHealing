...
module.exports = class SelfHealingHelper extends Helper {
  async _failed(test) {
    ...
    if (errMsg.includes("Clickable") || errMsg.includes("Locator")) {
      ...
      for (let i = test.steps.length; i >= 0; i--) {
...
console.log(`[Self-Healing]  if condition for retry`, typeof I[name] === 'function');
      if (typeof I[name] === 'function') {
        ...
        const currentIndex = failedSteps.findIndex(s => s.name === name && JSON.stringify(s.args) === JSON.stringify(args));
        console.log(`[Self-Healing] Current step index: ${currentIndex}`);
        // Execute all consequent steps
        for (let i = currentIndex; i < failedSteps.length; i++) {
          const stepToRun = failedSteps[i];
          const stepName = stepToRun.name;
          const stepArgs = [...stepToRun.args];
          if (typeof I[stepName] === 'function') {
            console.log(`[Self-Healing]  if condition for retry inside for`, typeof I[stepName] === 'function');

            await I[stepName](...stepArgs);
            console.log(`[Self-Healing] Successfully retried step: ${stepName} with args: ${JSON.stringify(stepArgs)}`);
          } else {
            console.warn(`[Self-Healing] Unknown step: ${stepName}. Skipping.`);
          }
        }
      } else {
